...
import { formatUnits, unitTotals, type Order } from './types';
...
export async function makeOrderPdf(order: Order, fontBytes: ArrayBuffer | Uint8Array) {
  ...
  function header() { page.drawRectangle({ x:left, y:y-26, width:tableWidth, height:30, color:rgb(.91,.94,.90) }); text('商品名稱',left+9,y-15); text('規格',left+245,y-15); text('數量',left+426,y-15); y -= 40; }
  ...
  text(`共 ${order.items.length} 項商品`,left,y,12);
  y-=24;
